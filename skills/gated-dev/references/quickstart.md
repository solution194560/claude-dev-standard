...
> ⚠️ **새 프로젝트에서 처음 한 번은 대화형으로 실행해 신뢰 대화상자를 승인하세요.**
> 워크스페이스가 "신뢰됨"으로 등록돼야 `.claude/settings.json`의 `allow` 규칙(테스트
> 실행 등)이 적용됩니다. 승인 전에는 4·5단계 게이트가 요구하는 `pytest`·실행 명령이
> 막혀 게이트가 "증거 불충분"으로 반려될 수 있습니다. 비대화형(`claude -p`) 실행도
> 이 승인이 끝난 뒤라야 정상 동작합니다.
